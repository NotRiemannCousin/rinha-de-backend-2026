...
#pragma pack(pop)
    ...
    FileRec rec;
    for (auto&[features, squaredNorm, isLegit] : g_dataset) {
        if (!file.read(reinterpret_cast<char*>(&rec), sizeof(rec))) break;
        features = rec.features;
        isLegit  = rec.isLegit;
        squaredNorm = 0;
        for (const float f : features) squaredNorm += f * f;
    }
}
...
[[nodiscard]] static int S_FraudNeighborCount(const std::array<float, 14>& q) noexcept {
    struct Neighbor { float dist; bool isLegit; auto operator<=>(const Neighbor&) const = default; };
    std::array<Neighbor, 5> top5;
    top5.fill({ std::numeric_limits<float>::infinity(), true });

    for (const auto&[features, squaredNorm, isLegit] : g_dataset) {
        float dot{};
        for (int j = 0; j < 14; ++j) dot += q[j] * features[j];

        if (const float dist{ squaredNorm - 2.0f * dot }; dist < top5.back().dist) {
            Neighbor mMax{ dist, isLegit };
            for (auto& top : top5) {
                auto [mn, mx]{ std::minmax(mMax, top) };
                top = mn;
                mMax = mx;
            }
        }
    ...
    return std::ranges::count(top5, false, &Neighbor::isLegit);
}
...
[[nodiscard]] static std::string_view S_ComputeFraudScore(
    ...
    std::println("Error: Erro ao fazer parse do JSON: {}", e.what());
    return S_FRAUD_RESPONSES[5];
}
...
static auto S_ServeLoop(ReceiverSocket& listener, Hermes::FastIoLoop& loop) {
    ...
    return listener.AsyncAcceptOne(opts)
         | stdexec::let_value([](auto&& clientSocket) {
               DBG_PRINTLN("API: +++ Novo socket TCP (FD) repassado com sucesso via SCM_RIGHTS! +++");
